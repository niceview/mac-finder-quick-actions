...
macOS Finder에서 파일이나 폴더를 우클릭해 **VS Code** 또는 **[herdr](https://github.com/)** 워크스페이스로 바로 여는 빠른 동작(Quick Action)과, 툴바/Dock에 올려두고 클릭이나 드래그 앤 드롭으로 쓰는 드롭릿 앱을 설치합니다.
...
| 드롭릿 `Open in VS Code.app` | `~/Applications/Open in VS Code.app` | 클릭하면 현재 Finder 창의 폴더를, 폴더를 드롭하면 그 폴더를 VS Code 새 창으로 연다 |
...
**툴바 드롭 대상** — `~/Applications` 의 드롭릿 앱을 **⌘ 누른 채** Finder 툴바로 드래그하면 아이콘이 고정됩니다. 사이드바(즐겨찾기)의 폴더를 그 아이콘 위로 드래그하면 열립니다.

> 사이드바 항목을 우클릭했을 때는 빠른 동작이 나타나지 않습니다. macOS가 사이드바에 별도의 축약 메뉴를 쓰기 때문이고, 우회할 방법이 없습니다. 드롭릿을 만든 이유가 이것입니다.

**아이콘 클릭 (Herdr)** — 현재 Finder 창의 폴더로 herdr 워크스페이스를 만듭니다. 폴더를 알아낼 수 없으면 이미 떠 있는 herdr 세션을 앞으로 가져오고, 꺼져 있으면 새로 띄워 attach 합니다.

**아이콘 클릭 (Open in VS Code)** — 현재 Finder 창의 폴더가 VS Code **새 창**으로 열립니다. 파일을 드롭하면 그 파일의 상위 폴더를 엽니다. Finder 창이 없거나 최근 항목·AirDrop 처럼 실제 폴더가 아닌 뷰라면 데스크탑을 엽니다.

> 각 드롭릿의 첫 클릭 때 "Finder 를 제어하도록 허용" 권한 팝업이 한 번 뜹니다. 허용해야 현재 폴더를 읽을 수 있습니다. 실수로 거부했다면 시스템 설정 → 개인정보 보호 및 보안 → **자동화** 에서 켜주세요.
...
| `DROPLET_NAME` | `Herdr` | herdr 드롭릿 앱 이름 |
| `VSCODE_DROPLET_NAME` | `Open in VS Code` | VS Code 드롭릿 앱 이름 |
...
드롭릿 두 개만 Finder 에 Apple Events 를 보냅니다. "현재 창의 폴더"는 Apple Events 없이는 알아낼 수 없기 때문입니다 (빠른 동작 쪽은 자동화 권한 팝업을 피하려고 전부 셸만 씁니다). 재설치하면 앱이 다시 빌드·서명되면서 서명 해시가 바뀌므로, 다음 클릭 때 권한 팝업이 다시 뜰 수 있습니다.
